...
def get_pair_pts(gt_contour, sp_pts, ins_pts, pair_id=None):
    ...
    threshhold = np.min([mean_dis * 2, tmp_pair[len(tmp_pair) // 2] * 2, 8])
...
def residual_rtt_allview(pars, w, h, f, offset, verts1, verts2, verts3, verts4, verts5, verts6, verts7, Crt1, Ct1, Crt2, Ct2,
                         Crt3, Ct3, Crt4, Ct4, Crt5, Ct5, Crt6, Ct6, Crt7, Ct7, pair_pts1, pair_pts2, pair_pts3, pair_pts4,
                         pair_pts5, pair_pts6, pair_pts7):
    residuals = []
    tp = np.array([pars['tx'], pars['ty'], pars['tz']])
    rtp = np.array([pars['rtx'], pars['rty'], pars['rtz']])
...
if __name__ == '__main__':
    ...
    img2_file_path = r'/home/sky7hate/Project/MultiviewFitting/data/observation/new_seg/bite/gt2n.jpg'
    img3_file_path = r'/home/sky7hate/Project/MultiviewFitting/data/observation/new_seg/bite/gt3n.jpg'
    img4_file_path = r'/home/sky7hate/Project/MultiviewFitting/data/observation/new_seg/bite/gt4n.jpg'
    img5_file_path = r'/home/sky7hate/Project/MultiviewFitting/data/observation/new_seg/bite/gt5n.jpg'
    img6_file_path = r'/home/sky7hate/Project/MultiviewFitting/data/observation/new_seg/bite/gt6n.jpg'
    img7_file_path = r'/home/sky7hate/Project/MultiviewFitting/data/observation/new_seg/bite/gt7n.jpg'
    ...
    rt7 = ch.array([1.55, -0.07, 0.05]) * np.pi / 4
    t7 = ch.array([0.018, 0.07, 2.25])
    w, h = (640, 480)
    f = w * 4 / 4.8
    ...
    rn7 = BoundaryRenderer()
    drn7 = DepthRenderer()
    crn7 = ColoredRenderer()

    rn7.camera = ProjectPoints(v=V_row_l, rt=rt7, t=t7, f=ch.array([f, f]),
                               c=ch.array([w, h]) / 2.,
                               k=ch.zeros(5))
    drn7.camera = ProjectPoints(v=V_row_l, rt=rt7, t=t7, f=ch.array([f, f]),
                                c=ch.array([w, h]) / 2.,
                                k=ch.zeros(5))
    crn7.camera = ProjectPoints(v=V_row_l, rt=rt7, t=t7, f=ch.array([f, f]),
                                c=ch.array([w, h]) / 2.,
                                k=ch.zeros(5))

    rn7.frustum = {'near': 1., 'far': 100., 'width': w, 'height': h}
    rn7.set(v=V_row_l, f=row_mesh_l.f, vc=row_mesh_l.vc * 0 + 1, bgcolor=ch.zeros(3), num_channels=3)
    drn7.frustum = {'near': 1., 'far': 100., 'width': w, 'height': h}
    drn7.set(v=V_row_l, f=row_mesh_l.f, vc=row_mesh_l.vc * 0 + 1, bgcolor=ch.zeros(3), num_channels=3)
    crn7.frustum = {'near': 1., 'far': 100., 'width': w, 'height': h}
    crn7.set(v=V_row_l, f=row_mesh_l.f, vc=row_mesh_l.vc, bgcolor=ch.zeros(3), num_channels=3)
    ...
    for iter in range(3):
        ...
        for i in range(numTooth+numTooth_l):
            ...
            while not (err < 1 or err_dif < 0.1 or iter > 20):
                ...
                out = lmfit.minimize(residual_rtt_allview, pars,
                                     args=(w, h, f, mean, trim_ins_pts1, trim_ins_pts2, trim_ins_pts3, trim_ins_pts4,
                                           trim_ins_pts5, trim_ins_pts6, trim_ins_pts7, rt1, t1, rt2, t2, rt3, t3, rt4, t4, rt5, t5,
                                           rt6, t6, rt7, t7, pair_pts1, pair_pts2, pair_pts3, pair_pts4, pair_pts5, pair_pts6, pair_pts7),
                                     method='leastsq')
                print('optimization time: %s s' % (time.time() - cur_time))

                total_pts = len(pair_pts1) + len(pair_pts2) + len(pair_pts3) + len(pair_pts4) + len(pair_pts5) + len(
                    pair_pts6) + len(pair_pts7)
                cur_time = time.time()
            ...
            if axarr is None:
                fig, axarr = plt.subplots(2, 3, sharex='col', sharey='row', gridspec_kw={'wspace': 0, 'hspace': 0})
    ...
    Mesh.save_to_obj('result/bite/V_row_bite_u1.obj', V_row, row_mesh.f)
    Mesh.save_to_obj('result/bite/V_row_bite_l1.obj', V_row_l, row_mesh_l.f)
    Mesh.save_to_obj('result/bite/V_row_bite1.obj', V_row_bite, merged_mesh.f)
